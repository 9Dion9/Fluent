...
**Where we are:** M0, M1, and M2 (both backend + iOS halves) built. `app/` is merged into this repo and pushed to GitHub. The iOS app now **compiles and runs on a real device** (iPhone 12) — it hit and fixed real compiler errors (missing `Combine` import, actor-isolation conflicts from `SWIFT_DEFAULT_ACTOR_ISOLATION = MainActor`, see Milestone log). The Worker is now **deployed to production**: `https://fluent-worker.dionmain.workers.dev` (deployed from the Mac via `wrangler deploy --config ../infra/wrangler.toml`; secrets `GATEWAY_SHARED_SECRET`/`TOKEN_SIGNING_KEY` uploaded via `wrangler secret put`). `app/Fluent/Networking/AppConfig.swift`'s `workerBaseURL` now points at that URL instead of `localhost:8787`, so the app works from a real device off the local network. **Not yet done:** the deployed Worker's `GATEWAY_URL` var is still the unroutable placeholder from `wrangler.toml` (`gateway.fluent.example.com`) — chat/TTS calls (M3+) will fail in production until a real reachable gateway URL is set as a Worker var/secret. The immediate next step is to pull this Worker-URL fix on the Mac, rebuild, and re-verify the onboarding flow end-to-end on-device (M2 verify step). See the Milestone log at the bottom for full per-milestone detail, and the "iOS app (M2, frontend half)" section below for what's actually in `app/`.
