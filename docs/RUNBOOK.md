...
**Where we are:** M0-M3 done and verified on a real device — real chat works end-to-end on the user's iPhone against the production Worker. Two real bugs found by on-device testing and fixed: (1) `reminder_time` was required-but-nullable in the profile-update zod schema, but Swift's Encodable omits nil-optional keys entirely rather than sending `null`, so skipping the onboarding reminder step always failed profile save with a bare "Required" error; (2) onboarding had no back navigation at all — fixed, back chevron next to the progress dots on every screen except Welcome and FirstMessage. Home currently *is* Chat (no tab bar / way to "exit" chat) — this is expected, not a bug, until Today/Words/Camera exist in M5-M7; user explicitly deprioritized further onboarding/nav polish until all functionality is built.

**M4 (voice messages) backend done, tested (31/31 vitest), and live-verified for real** against the running gateway on this box (see below) — iOS half written but NOT YET compiled/run on-device.
