...
**Where we are:** M0-M2 fully built and verified on a real device (onboarding got a first design pass: soft blob backdrop + tutor persona avatars, more polish deliberately deferred to end-of-project per user decision). **M3 (tutor chat, text) backend is built, tested (21/21 vitest), and eval-verified (26/27, see below) — iOS half is written but NOT YET compiled/run on-device.** The Worker is deployed to production: `https://fluent-worker.dionmain.workers.dev`.

**Blocking next step before M3 can be called done:** two things need to happen on the Mac/Cloudflare side, in order:
1. Apply migration `0002_conversation_summary.sql` to the **remote** D1 database (adds `conversations.summary` + `summarized_through_message_count`) — it's only applied locally so far. Run from `infra/`: `CLOUDFLARE_API_TOKEN=... ./migrate.sh --remote`.
2. Set the deployed Worker's `GATEWAY_URL` to the real `cloudflared` tunnel URL (still the unroutable `gateway.fluent.example.com` placeholder in production) via `npx wrangler deploy --var GATEWAY_URL:<tunnel-url> --config ../infra/wrangler.toml` or `wrangler secret put` — without this, every `/v1/chat` call in production 503s with `tutor_napping` (fails fast by design, but chat simply won't work until this is set). Remember the quick-tunnel URL changes every restart (see gotcha below) — check it's still live first.

Then: pull on the Mac, build, and manually verify a real chat exchange on-device (M3 verify step: 10-minute conversation, never leaves scope).
